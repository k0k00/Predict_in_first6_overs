...
```bash
vamsi-app-v1$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
vamsi-app-v1$ sam local start-api
vamsi-app-v1$ curl http://localhost:3000/classify_digit
```
...
```bash
vamsi-app-v1$ sam logs -n InferenceFunction --stack-name vamsi-app-v1 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name vamsi-app-v1
```
